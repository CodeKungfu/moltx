    ...
    <section class="card">
      ...
      <div class="list" style="margin-top: 16px;">
        ${
          pending.length > 0
            ? html`
              <div class="muted" style="margin-bottom: 8px;">待定</div>
...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const age = typeof req.ts === "number" ? formatRelativeTimestamp(req.ts) : "不可用";
        ...
        state.formMode === "raw"
          ? html`
              <div class="callout warn" style="margin-top: 12px">
                切换配置标签页到 <strong>表单</strong> 模式以在此处编辑绑定。
              </div>
            `
          : nothing
      }
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          ...
          <div class="card-sub">
            针对 <span class="mono">exec host=gateway/node</span> 的允许列表和批准策略。
          </div>
        </div>
        <button
          class="btn"
      ...
      ${
        !ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">加载执行批准以编辑允许列表。</div>
    ...
    <div class="list" style="margin-top: 12px;">
      ...
      ${
        state.target === "node" && !hasNodes
          ? html`
              <div class="muted">尚无节点通告执行批准。</div>
            `
          : nothing
      }
    </div>
...
function renderAllowlistEntry(
  ...
  const lastUsed = entry.lastUsedAt ? formatRelativeTimestamp(entry.lastUsedAt) : "从不";
