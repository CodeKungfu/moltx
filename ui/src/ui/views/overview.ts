...
export function renderOverview(props: OverviewProps) {
  const snapshot = props.hello?.snapshot as
    ...
    | undefined;
  const uptime = snapshot?.uptimeMs ? formatDurationHuman(snapshot.uptimeMs) : "不可用";
  const tick = snapshot?.policy?.tickIntervalMs ? `${snapshot.policy.tickIntervalMs}ms` : "不可用";
  const authHint = (() => {
      ...
      return html`
        <div class="muted" style="margin-top: 8px">
          ...
          <div style="margin-top: 6px">
            <span class="mono">openclaw dashboard --no-open</span> → 打开控制 UI<br />
  ...
  const insecureContextHint = (() => {
    ...
    return html`
      <div class="muted" style="margin-top: 8px">
        此页面为 HTTP，浏览器会阻止设备身份。请使用 HTTPS (Tailscale Serve) 或在网关主机上打开
  ...
  return html`
    <section class="grid grid-cols-2">
      <div class="card">
        ...
        <div class="card-sub">控制面板连接的位置及其认证方式。</div>
        ...
        <div class="stat-grid" style="margin-top: 16px;">
          ...
          <div class="stat">
            ...
            <div class="stat-value">
              ${props.lastChannelsRefresh ? formatRelativeTimestamp(props.lastChannelsRefresh) : "不可用"}
            </div>
          </div>
        </div>
        ${
    ...
    <section class="grid grid-cols-3" style="margin-top: 18px;">
      ...
      <div class="card stat-card">
        ...
        <div class="stat-value">${props.sessionsCount ?? "不可用"}</div>
        <div class="muted">网关跟踪的最近会话密钥。</div>
      </div>
      <div class="card stat-card">
        <div class="stat-label">Cron</div>
        <div class="stat-value">
          ${props.cronEnabled == null ? "不可用" : props.cronEnabled ? "已启用" : "已禁用"}
        </div>
    ...
    <section class="card" style="margin-top: 18px;">
      ...
      <div class="note-grid" style="margin-top: 14px;">
        <div>
          ...
          <div class="muted">
            首选 serve 模式，以通过 tailnet 认证将网关保持在 loopback 上。
          </div>
        </div>
        <div>
